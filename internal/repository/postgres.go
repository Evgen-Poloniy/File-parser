...
func (p *PostgresRepository) InsertParsedData(ctx context.Context, filename string, devices []entity.Device, deviceData []entity.DeviceData) error {
	...
	stmt, err := tx.PrepareContext(ctx, `
		INSERT INTO device_data (
			n, mqtt, msg_id, text, context, class, level, area, addr, block, type, bit, invert_bit, device_id
...
func (p *PostgresRepository) GetDataByUnitGUID(ctx context.Context, unitGUID string, limit int, page int) ([]dto.Response, error) {
	...
	for rows.Next() {
		var r dto.Response
		err := rows.Scan(
			&r.N, &r.MQTT, &r.InvID, &r.UnitGUID,
			&r.MsgID, &r.Text, &r.Context, &r.Class,
			&r.Level, &r.Area, &r.Addr, &r.Block,
			&r.Type, &r.Bit, &r.InvertBit,
		)
		if err != nil {
			return nil, err
		}
...
func (p *PostgresRepository) InsertErrors(ctx context.Context, filename string, parseErrors *entity.ParseErrors) error {
	...
	err = tx.QueryRowContext(
		ctx,
		`
		INSERT INTO not_parsed_files(filename)
		VALUES($1)
		RETURNING id
		`,
		filename,
